...
export class RoomStore {
  ...
  queueReflection(r){const endReason=r.game?.endReason||'normal';if(r.game?.status!=='finished'||endReason!=='normal'||!r.gameId||!r.players.some(p=>p.ai&&p.mode==='llm-advanced'))return;const snapshot=createEndSnapshot(r.game,{gameId:r.gameId,players:r.players});r.reflectionStatus={state:'syncing',status:'syncing'};void this.reflection.enqueue(snapshot).then(()=>this.reflection.recoverPending({key:this.llmConfig.apiKey})).then(results=>{const failed=results.some(item=>item?.status==='failed');r.reflectionStatus={state:failed?'failed':'saved',status:failed?'failed':'saved',lessons:results.reduce((n,item)=>n+(item?.lessons||0),0)};this.publish(r);}).catch(()=>{r.reflectionStatus={state:'failed',status:'failed'};this.publish(r);});}
