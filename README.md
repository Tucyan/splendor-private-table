...
- 实时显示在线/离线，自动重连。断线保留席位，轮到离线玩家时等待；全部真人离线时保留局面并暂停 AI，无人在线满 12 小时后自动关闭房间。房主可移出离线玩家，由本地策略接管正在进行的席位。主动离开也会托管；房主离开后转交下一位真人，最后一位真人主动离开时立即关闭房间。
...
- 手机竖屏将玩家放在顶部、市场居中、拿取/返还区放在底部；横屏使用玩家、市场、操作区三列。在市场区域横向滑动，或使用“市场 / 预留 / 宝石库”分段按钮，可切换预留手牌和宝石库详情；底部持有摘要始终占满库存栏。主牌桌使用动态视口高度及安全区域，不依靠页面滚动寻找行动按钮；房间管理和规则从右上角菜单进入。详见 [手机适配说明](docs/mobile-layout.md)。
- 卡牌根据自身筹码、黄金和永久折扣持续高亮：自己的可行动回合显示“可购买”，其他回合显示“可负担”。桌面悬停或手机长按资源不足的卡牌会显示颜色缺口、黄金可抵扣量与最终仍缺总数；足够购买时不弹提示。
- 贵族要求使用彩色发展卡图标表示。悬停或聚焦贵族会显示各颜色还缺的永久卡数量，并说明贵族价值 3 分、在回合结束满足全部永久折扣要求后获得。
...
- 关键桌面动作带有克制的本地音效：开局洗牌、拿取宝石、购买/预留卡牌、贵族来访、轮到自己与结算。页头扬声器按钮可随时关闭，选择会保存在当前浏览器；首次交互前不会尝试自动播放。音效来自 Kenney Casino Audio 1.1，采用 CC0 许可，详见 [资产说明](docs/assets.md)。
...
规则与卡池来源见 [卡牌数据说明](docs/card-data-sources.md)，美术与音频来源见 [资产说明](docs/assets.md)。本项目使用原创美术，未复制原桌游插画；名称与基础规则用于私人游玩。
